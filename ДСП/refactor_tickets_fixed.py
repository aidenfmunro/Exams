...
def process_file(filepath):
    ...
    for part in parts:
        if part.startswith('## '):
            ...
            if "Основные теоремы и свойства" in current_section or "Примеры" in current_section or "Дополнительные вопросы" in current_section or "определения" in current_section:
                # Replace **Text** at the start of a line
                part = re.sub(r'^\*\*(.*?)\*\*(.*)', repl_sec4, part, flags=re.MULTILINE)
            
            if "Полный ответ" in current_section:
                # Check if it already has ### 2.1
                if "### 2." not in part:
...
"""
                    ...
                    if result.returncode == 0 and result.stdout.strip():
                        ...
                        print(f"Warning: LLM failed for {filepath}, keeping original.")
